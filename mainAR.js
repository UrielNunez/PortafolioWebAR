...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function handleOrientation(event) {
        ...
        const rotateX = Math.min(Math.max(beta - 50, -80), 80);
        const rotateY = Math.min(Math.max(gamma, -80), 80);
        card.style.transform = `rotateX(${rotateX * -2.5}deg) rotateY(${rotateY * 2.5}deg)`;
    }
                ...
                DeviceOrientationEvent.requestPermission()
                    .then(permissionState => {
                        if (permissionState === 'granted') {
                            ...
                            gyroPermissionDiv.style.display = 'none'; // ocultar aviso
                        } else {
                            alert('Permiso denegado, no se podrá activar el giroscopio.');
                        }
                    })
                    .catch(console.error);
            } else {
                // Android o navegadores sin permiso requerido
                window.addEventListener('deviceorientation', handleOrientation);
                gyroPermissionDiv.style.display = 'none'; // ocultar aviso
            }
        } else {
            alert('Tu navegador no soporta giroscopio.');
            gyroPermissionDiv.style.display = 'none'; // ocultar aviso
        }
    }
